...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	{
		trace('Yeah this is for charting');
		...
		for(i in array)
			{
				...
				var section = 0;
				//Search where this note should be placed
				for(ii in _song.notes)
				{
					if (ii.startTime <= strum && ii.endTime > strum)
					{
						...
						if(i[4] != null)
						{
							newData = [strum,i[1],i[2],i[4]];
						}
						else
						{
							newData = [strum,i[1],i[2]];
						}
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.mouse.justPressedRight && !waitingForRelease)
		{
			if (FlxG.mouse.x > 0
			...
			{
				FlxG.log.add("Special Note Added");
				addCheeseNote();
			}
		}
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var daStrumTime = i[0];
				var daSus = i[2];
				var daNoteType:Null<Int> = i[3];
				if(daNoteType == null) 
				{
					daNoteType = 0;
				}
				var note:Note = new Note(daStrumTime, daNoteInfo % 4,null,false,true,false,daNoteType);
	...
	private function addCheeseNote():Void
	{
		...
		var noteSus = 0;
		var note:Note = new Note(noteStrum, noteData % 4,null,false,true,false,1);
		addNote(note);
	}
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n != null)
			...
			section.sectionNotes.push([noteStrum, noteData, noteSus,0]);
	...
	private function saveLevel()
	{
		...
		var data:String = Json.stringify(json,null,null);
