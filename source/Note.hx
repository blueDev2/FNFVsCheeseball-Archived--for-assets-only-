...
class Note extends FlxSprite
{
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?isAlt:Bool = false, ?noteType:Int = 0)
	{
		super();
		...
		if (inCharter)
		{
			...
			for (i in 0...4)
			{
				...
				animation.addByPrefix(dataColor[i] + 'cheeseBoom', dataColor[i] + ' special'); // Special Note
			}
		...
		if(noteType == 1)
		{
			animation.play(dataColor[noteData] + 'cheeseBoom');
		}
		else
		{
		...
		if (FlxG.save.data.stepMania && !isSustainNote)
		{
			...
			if(noteType == 1)
			{
				animation.play(dataColor[col] + 'cheeseBoom');
			}
			else
			{
