...
class PlayState extends MusicBeatState
{
	...
	private var p1FramesNum:Int;
	private var p2FramesNum:Int;
	...
	var cheeseSpectators1:FlxSprite;
	var cheeseSpectators2:FlxSprite;
	var cheeseSpray:FlxSprite;
	var cheeseSprayTween:FlxTween;
	...
	//Songs with cutscenes (A song has a cutscene if it is preceded by that cutscene)
	...
	override public function create()
	{
		...
		var hasOldDialogue = false;
		var hasNewDialogue = false;
		var oldDialogue:Array<String> = null;
		var newDialoguePath:String = "";
		switch(songLowercase)
		{
			//if the song has dialogue, so we don't accidentally try to load a nonexistant file and crash the game
			case 'senpai' | 'roses' | 'thorns':
				oldDialogue = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue'));
				hasOldDialogue = true;
			case '':
				newDialoguePath = Paths.txt('data/'+ songLowercase+'/dialogue');
			...
			switch (stageCheck)
			{
				...
				case 'schoolEvil':
					...
					case 'cheese':
					{
						...
						var cheeseBg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cheeseBackground','weekCheese'));
						cheeseBg.antialiasing = FlxG.save.data.antialiasing;
						cheeseBg.scrollFactor.set(0.9, 0.9);
						cheeseBg.setGraphicSize(Std.int(cheeseBg.width * 1.1));
						cheeseBg.updateHitbox();
						cheeseBg.active = false;
                        add(cheeseBg);
                    
						var cheeseFg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cheeseForeground','weekCheese'));
						cheeseFg.antialiasing = FlxG.save.data.antialiasing;
						cheeseFg.scrollFactor.set(0.9, 0.9);
						cheeseFg.setGraphicSize(Std.int(cheeseFg.width * 1.1));
						cheeseFg.updateHitbox();
						cheeseFg.active = false;
                        add(cheeseFg);

						fountain = new FlxSprite(1200, 150);
						fountain.frames = Paths.getSparrowAtlas('fountain','weekCheese');
						fountain.animation.addByPrefix('idle', "fountain", 24, false);
						fountain.antialiasing = FlxG.save.data.antialiasing;
						fountain.scrollFactor.set(0.9, 0.9);
						fountain.setGraphicSize(Std.int(fountain.width * 1.3));
						fountain.updateHitbox();
						add(fountain);


						cheeseSpectators1 = new FlxSprite(-200, 350);
						cheeseSpectators1.frames = Paths.getSparrowAtlas('back_spectators','weekCheese');
						cheeseSpectators1.animation.addByPrefix('idle', "bop", 24, false);
						cheeseSpectators1.antialiasing = FlxG.save.data.antialiasing;
						cheeseSpectators1.scrollFactor.set(0.9, 0.9);
						cheeseSpectators1.setGraphicSize(Std.int(cheeseSpectators1.width * 1.5));
						cheeseSpectators1.updateHitbox();
						//cheeseSpectators.active = false;
						add(cheeseSpectators1);
		...
		// Layering of things over the characters
		if(stageCheck == 'cheese')
		{
			cheeseSpectators2 = new FlxSprite(-200, 450);
			cheeseSpectators2.frames = Paths.getSparrowAtlas('front_spectators','weekCheese');
			cheeseSpectators2.animation.addByPrefix('idle', "bop", 24, false);
			cheeseSpectators2.antialiasing = FlxG.save.data.antialiasing;
			cheeseSpectators2.scrollFactor.set(0.9, 0.9);
			cheeseSpectators2.setGraphicSize(Std.int(cheeseSpectators1.width * 1.5));
			cheeseSpectators2.updateHitbox();
			add(cheeseSpectators2);
		}
		...
		var doof:Dynamic = new OldDialogueBox(null);
		if(hasOldDialogue)
		{
			doof = new OldDialogueBox(oldDialogue,this);
			// doof.x += 70;
			// doof.y = FlxG.height * 0.5;
			doof.scrollFactor.set();
			//doof.finishThing = startCountdown;
		}
		else if(hasNewDialogue)
		{
			doof = new DialogueBox(newDialoguePath,this);
		}	
		...
		p1FramesNum = iconP1.numFrames;
		add(iconP1);

		iconP2 = new HealthIcon(SONG.player2, false);
		iconP2.y = healthBar.y - (iconP2.height / 2);
		p2FramesNum = iconP2.numFrames;
		add(iconP2);
		...
		if(curStage == 'cheese')
			{
				...
				cheeseSpray = new FlxSprite(0,0).loadGraphic(Paths.image('cheeseSpray','weekCheese'));
				cheeseSpray.alpha = 0;
				cheeseSpray.antialiasing = FlxG.save.data.antialiasing;
				cheeseSpray.setGraphicSize(Std.int(FlxG.width * 1.3));
				cheeseSpray.scrollFactor.set(0.9, 0.9);
				cheeseSpray.updateHitbox();
				cheeseSprayTween = FlxTween.tween(cheeseSpray,{alpha: 0.9},0.7,{type:PERSIST,onComplete:function(NULL){
					FlxTween.tween(cheeseSpray,{alpha: 0},1,{type:PERSIST});
				} });
				cheeseSprayTween.active = false;
				add(cheeseSpray);
				cheeseSpray.cameras = [camHUD];
			}
		super.create();
	}

	function schoolIntro(?dialogueBox:OldDialogueBox):Void
	{
	...
	public function generateSong(dataPath:String):Void
	{
		...
		if (true)//paused)
		{
			...
			if (!isStoryMode && isSM)
			{
				...
				trace("Background music added1");
			}
			else
			{
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 1, false);
				trace("Background music added2");
			}
			#else
			FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 1, false);
			trace("Background music added3");
			#end
		}
		trace("Paused: " + paused);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var daNoteType:Null<Int> = songNotes[3];

				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote,false,false,false,daNoteType);
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				default:
					...
					for (j in 0...4)
					{
						//babyArrow.animation.addByPrefix(dataColor[j], 'arrow' + dataSuffix[i]);	
						babyArrow.animation.addByPrefix(dataColor[j], dataColor[j] + ' special');	
					}
							...
							if (loadRep && daNote.isSustainNote)
							{
								...
								else
								{
									...
									else
									{
										...
										{
											noteMissAnimAndSound(daNote.noteData);
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if(Assets.exists(Paths.txt("data/" + PlayState.SONG.song+"/cutscene","preload"),AssetType.TEXT))
					{
						Cutscene.filePath = Paths.txt("data/" + PlayState.SONG.song+"/cutscene","preload");
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		//special note control
		if(daNote.noteType == 1)
		{
			//cheesespray animation
			//if(!cheeseSprayTween.active)
			{
				cheeseSprayTween.start();
			}
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			noteMissAnimAndSound(direction);
	...
	function noteMissAnimAndSound(direction:Int = 1)
	{
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'cheese':
				if (FlxG.save.data.distractions)
				{
					cheeseSpectators1.animation.play('idle',true);
					cheeseSpectators2.animation.play('idle',true);
					fountain.animation.play('idle',true);
				}
			case "philly":
				if (FlxG.save.data.distractions)
				{
			...
			default:
				//cheeseSpectators.animation.play('idle',true);
